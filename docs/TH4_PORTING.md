...
- **Phase 3 — `.STD` (parser + timeline DONE):** `stage.rs` parses the `.STD` file into map-section scroll order, per-section scroll speeds, enemy scripts (≤32), and the stage-timeline bytecode. `timeline_events()` decodes the timeline (reversed from `std_run` asm in `th04_main.asm:16959`): records of `u16 frame`, `u8 count`, `count × {u8 script, i16 x, i16 y, u8 arg}` (8 bytes each), `frame==0` terminates. Verified against `ST00..ST06.STD` (consumes every byte; ST00 = 105 spawn-frames / 246 enemies, e.g. script#0 wave at frames 272–336 entering from y=−256). **Next:** the per-enemy script VM (drives movement + bullet patterns; in asm/partly un-RE'd), then a stage harness reusing TH06's collision/scoring/loop.
