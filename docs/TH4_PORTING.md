...
- **Phase 3 — `.STD` (parser + timeline + enemy-script disasm DONE):**
  ...
  - **The full data + gameplay spine of TH04 now runs in Rust, headless.** Remaining for a visible build: sprite rendering (BB/BFT tile parsers + stage CDG backgrounds + palettes), wiring `StageSim` into `th06-engine` (draw enemies/bullets/player/bg each frame), and the WASM glue (reuse TH06 `web.rs`).
