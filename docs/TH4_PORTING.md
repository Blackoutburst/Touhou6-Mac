...
- **Phase 3 — `.STD` (parser + timeline + enemy-script disasm DONE):**
  ...
  - **Next:** collision (player shots ↔ enemies for damage/score, enemy bullets ↔ player) tying the timeline + enemy VM + bullets + player into a headless stage simulation; then sprite rendering (BB/BFT tiles + stage CDG backgrounds + palettes) to draw it.
