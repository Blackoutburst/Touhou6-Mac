...
- **Phase 3 — `.STD` (parser + timeline + enemy-script disasm DONE):**
  ...
  - **Next:** the stateful enemy VM (execute the opcodes — apply motion, tick blocking-frame counters, spawn bullets via the bullet system in `th04/main/bullet/`), the player, collision/scoring (reuse TH06 template), and sprite rendering (BB/BFT tiles).
