...
- **Phase 3 — `.STD` (parser + timeline + enemy-script disasm DONE):**
  ...
  - **Next:** the bullet system (`th04/main/bullet/` — spawn objects from the template + autofire interval), the player (`th04/main/player/`), collision/scoring (reuse TH06 template), and sprite rendering (BB/BFT tiles + stage CDG backgrounds + palettes).
